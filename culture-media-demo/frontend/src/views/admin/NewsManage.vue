<template>
  <div>
    <h2>新闻管理</h2>
    <el-button type="primary" @click="open()">新增新闻</el-button>
    ...
    <el-dialog v-model="visible" :title="form.id ? '编辑新闻' : '新增新闻'" width="700px">
...
const list = ref([]); const visible = ref(false);
const blank = () => ({ id: null, title: '', summary: '', cover_url: '', content: '', publish_time: '', status: 1 });
const form = reactive(blank());

const load = async () => { const res = await request.get('/admin/news'); list.value = res.data; };
load();
const open = (row) => { Object.assign(form, row || blank()); visible.value = true; };
const save = async () => {
  if (form.id) await request.put(`/admin/news/${form.id}`, form);
  else await request.post('/admin/news', form);
  ElMessage.success('保存成功'); visible.value = false; load();
};
const remove = async (id) => { await ElMessageBox.confirm('确认删除?'); await request.delete(`/admin/news/${id}`); ElMessage.success('删除成功'); load(); };
</script>
